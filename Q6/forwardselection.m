...
% y = csvread('FTSE.csv',1,0,[1 0 749 0]);
y = csvread('FTSE.csv',2,0,[2 0 750 0]);
y_sub = csvread('FTSE.csv',1,0,[1 0 749 0]);
y = (y - y_sub) ./ y;


X = horzcat(AAL_1, HSBA_2, ADM_3, ANTO_4, CCH_5, CNA_6, CPG_7, CRDA_8,CRH_9, HSBA_10, IHG_11,IMB_12,INF_13,ITRK_14,NMC_15,NXT_16, OML_17, PPB_18,PSON_19,SGE_20,SGRO_21,SHPG_22,SKY_23,UU_24,SVT_25,VOD_26,WTB_27,BATS_28,FERG_29,RIO_30);

train = @(x_train, y_train, x_test, y_test) sum((y_test - x_test*(x_train \ y_train)).^2);

% train = @(x_train, x_test, y_train, y_test) sum((y_test - x_test*(x_train \ y_train)).^2);
...
X = horzcat(CPG_7, PPB_18, SGE_20, UU_24, SVT_25, FERG_29);
tot = 0.0113 + 0.0113 + 0.0286 + 0.0286;

first = 0.0113 / tot;
second = 0.0286 / tot;
X2 = horzcat(PPB_18 .* first, SGE_20 .* first, SVT_25 .* second, FERG_29 .* second);
...
% FTSE_exp_return = mean(y);
% exp_return_fifth = (mean(HSBA_2) + mean(CCH_5) + mean(NMC_15) + mean(PPB_18) + mean(PSON_19) + mean(SHPG_22)) / 6;
% 
% bar(counter)
% 
% title('Greedy Forward Selection Algorithm')
% 
% xlabel('Stocks') % x-axis label
% ylabel('Number of times selected') % y-axis label
